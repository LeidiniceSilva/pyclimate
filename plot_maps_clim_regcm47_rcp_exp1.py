...
def import_rcm(var, area, exp, dt):
	...
	std_ann = np.std(value[0:240:4,:,:], axis=0)
	...
	ann_rcm = np.nanmean(value[0:240:4,:,:], axis=0)
...
def import_gcm(var, area, exp, dt):
	...
	ann_gcm = np.nanmean(value[0:240:4,:,:], axis=0)
...
cbar = map.colorbar()
cbar.ax.tick_params(labelsize=6)   
cbar.set_label('Precipitation \n change (mm d⁻¹)', size=6, labelpad=5, fontweight='bold', rotation=90)
map.drawmeridians(np.arange(-85.,-5.,20.), size=6, labels=[0,0,0,0], linewidth=0.4, color='black')
map.drawparallels(np.arange(-20.,15.,10.), size=6, labels=[0,0,0,0], linewidth=0.4, color='black')
...
plt.ylabel(u'Latitude', fontsize=6, labelpad=15, fontweight='bold')
map, xx, yy = basemap(lat, lon)
plot_maps_diff = map.contourf(xx, yy, pre_ann_rcm_rcp26_hist, levels=levs2, latlon=True, cmap=cm.BrBG)
...
cbar = map.colorbar()
cbar.ax.tick_params(labelsize=6)   
cbar.set_label('Temperature \n change (°C)', size=6, labelpad=5, fontweight='bold', rotation=90)
map.drawmeridians(np.arange(-85.,-5.,20.), size=6, labels=[0,0,0,0], linewidth=0.4, color='black')
map.drawparallels(np.arange(-20.,15.,10.), size=6, labels=[0,0,0,0], linewidth=0.4, color='black')
