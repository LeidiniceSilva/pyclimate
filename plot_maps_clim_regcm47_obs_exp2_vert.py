...
def import_gcm(var, area, exp, dt):
	...
	arq  = '{0}/{1}_{2}_Amon_HadGEM2-ES_{3}_r1i1p1_djf_{4}_lonlat_lev.nc'.format(path, var, area, exp, dt)	
...
lat, hur_lev_rea, hur_var_rea = import_obs('r', 'amz_neb', '1986-2005')
lat, hur_lev_rcm, hur_var_rcm = import_rcm('rh', 'amz_neb', 'historical', '1986-2005')
lat, hur_lev_gcm, hur_var_gcm = import_gcm('hur', 'amz_neb', 'historical', '1986-2005')

print(lat.shape, hur_lev_rea.shape, hur_var_rea.shape)
print(lat.shape, hur_lev_rcm.shape, hur_var_rcm.shape)
print(lat, hur_lev_gcm.shape, hur_var_gcm.shape)

# Plot models and obs database 
fig = plt.figure(figsize=(8,6))
time = np.arange(-85, -10, 5)

ax = fig.add_subplot(3, 1, 1)
plt.title(u'A)', loc='left', fontsize=8, fontweight='bold')
plt1 = ax.contourf(lat, hur_lev_rea, hur_var_rea, np.arange(10, 90, 10), cmap=plt.cm.Blues, extend='max')
plt.colorbar(plt1, aspect=10, pad=0.025)
plt2 = ax.contour(lat, hur_lev_rea, hur_var_rea, np.arange(10, 90, 10), linewidths=0.5, colors='black')
...
plt1 = ax.contourf(lat, hur_lev_rcm, hur_var_rcm, np.arange(10, 90, 10), cmap=plt.cm.Blues, extend='max')
plt.colorbar(plt1, aspect=10, pad=0.025)
plt2 = ax.contour(lat, hur_lev_rcm, hur_var_rcm, np.arange(10, 90, 10), linewidths=0.5, colors='black')
ax.clabel(plt2, fmt='%d', fontsize=8, colors='black')
plt.ylabel('Pressure (hPa)', fontweight='bold')
plt.ylim(100, 1000)
plt.xticks(time, ('85°W', '80°W', '75°W', '70°W', '65°W', '60°W', '55°W', '50°W', '45°W', '40°W', '35°W', '30°W', '25°W', '20°W', '15°W'), fontsize=8)
...
plt1 = ax.contourf(lat, hur_lev_gcm, hur_var_gcm, np.arange(10, 90, 10), cmap=plt.cm.Blues, extend='max')
plt.colorbar(plt1, aspect=10, pad=0.025)
plt2 = ax.contour(lat, hur_lev_gcm, hur_var_gcm, np.arange(10, 90, 10), linewidths=0.5, colors='black')
ax.clabel(plt2, fmt='%d', fontsize=8, colors='black')
